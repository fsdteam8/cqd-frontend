...
const formSchema = z.object({
  ...
  keywords: z.array(z.string()).optional(), // Make this optional or min(1) based on your needs
...
export default function EditBlogPage({ initialData }: { initialData: string }) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    mutationFn: (formData: FormData) => {
      ...
      return fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/blogs/${initialData}`,
        {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <div className="flex justify-end mt-[30px] mb-[208px]">
              <button
                className="text-xl font-medium leading-[120%] tracking-[0%] text-[#F4F4F4] py-[13px] px-[26px] rounded-[8px] bg-[#0E2A5C] disabled:opacity-50"
                type="submit"
                disabled={isPending}
              >
